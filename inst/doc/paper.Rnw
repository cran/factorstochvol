...
\newcommand{\betapriorf}{b_\beta}
\newcommand{\betapriors}{B_\beta}
...
However, due to the large number of latent quantities, their efficient estimation is non-trivial and software that allows to easily fit SV models to data is rare.
  We aim to alleviate this issue by presenting novel implementations of four SV models delivered in two \proglang{R} packages.
  Several unique features are included and documented. As opposed to previous versions, \stochvol{} is now capable of handling linear mean models, heavy-tailed SV, and SV with leverage.
...
First, we extend \stochvol{}~\citep{kastner2019stochvol} with several practically relevant univariate methods.
Second, we introduce the new companion package \factorstochvol~\citep{factorstochvol} which focuses on the multivariate case.
The extended \stochvol{} now provides the means for the Bayesian estimation of vanilla SV, heavy-tailed SV and SV with leverage~\citep{harvey1996estimation,omori2007stochastic,nakajima2012stochastic}.
...
Consequently, we proceed with two relaxed models: the SV model with Student's $t$~errors (SVt) and the SV model with leverage (SVl).
...
\begin{equation}\label{eq:vanillasv}
  ...
  \end{split}
\end{equation}
where $\Normal{a}{b}$ denotes the normal distribution with mean $a$ and variance $b$.
...
The $\nreg$ regression coefficients are collected in $\bm\beta=(\beta_1,\dots,\beta_\nreg)^\top$, and $\hvec=(\hpar_1,\dots,\hpar_\leny)^\top$ denotes the log-variance process.
...
In the following, we describe the SV model with Student's $t$~errors implemented in \stochvol.
\begin{equation}\label{eq:heavysv}
  \begin{split}
    y_t\mid\bm x_t,\bm\beta,\hpar_t,\nupar &\sim \Student{\nupar}{\bm x_t\bm\beta}{\exp(\hpar_t/2)}, \\
    \hpar_{t+1}\mid\svpars,\hpar_t &\sim \Normal{\mupar+\phipar(\hpar_t-\mupar)}{\sigmapar^2},
  \end{split}
\end{equation}
where $\Student{\nupar}{a}{b}$ is the Student's $t$~distribution with $\nupar$ degrees of freedom, mean $a$, and scale $b$.
The single difference between Equation~\ref{eq:vanillasv} and Equation~\ref{eq:heavysv} is that the observations are conditionally $t$~distributed.
Hence, Equation~\ref{eq:heavysv} relaxes Equation~\ref{eq:vanillasv} through the new parameter $\nupar$ as the Student's $t$~distribution converges in law to the standard normal distribution when $\nupar$ goes to infinity.
%
...
\begin{equation}\label{eq:svleverage}
  ...
  \end{split}
\end{equation}
where $\Normal[2]{\bm b}{\bm B}$ is the two-dimensional normal distribution with mean vector $\bm b\in\Real^2$ and covariance matrix $\bm B\in\Real^{2\times2}$.
...
%
We a priori assume $\beta_k\sim\Normal{\betapriorf}{\betapriors}$ independently for $k=1,\dots,\nreg$.
%, where $\Normal{\betapriorf}{\betapriors}$ is the normal distribution with mean $\betapriorf$ and variance $\betapriors$.
By setting $\betapriorf=0$, the prior distribution becomes the Bayesian analogue to ridge regression~\citep[see, e.g.,][for a discussion of this and other shrinkage priors]{park2008bayesian}.
It allows strong prior information and thus shrinkage toward $\betapriorf$ for small values of $\betapriors$, but it can also serve as a rather uninformative prior for large values of $\betapriors$.
...
Such domain knowledge can be used as prior information by allocating more probability to positive high values of $\phipar$, e.g., by setting $\phipriorf\approx1.5$ and $\phipriors\gtrsim5$.
...
The commonly applied and convenient conjugate prior $\sigmapar^{-2}\sim\Gammadist{a_\sigmapar}{b_\sigmapar}$, where $\Gammadist{a_\sigmapar}{b_\sigmapar}$ is the gamma distribution with shape parameter $a_\sigmapar$ and rate parameter $b_\sigmapar$, bounds $\sigmapar$ away from 0 and it is therefore in our view an unsatisfactory choice.
Following~\citet{fruhwirth2010stochastic} and \citet{kastner2014ancillarity}, we advocate and employ $\pm\sigmapar\sim\Normal{0}{\sigmaprior}$ instead, which corresponds to $\sigmapar^2\sim\Gammadist{1/2}{1/(2\sigmaprior)}$.
...
Equation~\ref{eq:heavysv} -- the SV model with Student's~$t$ errors -- additionally requires the prior specification of the degrees of freedom parameter $\nupar$.
To ascertain interpretability of the scaling $\exp(\hpar_t/2)$, we ensure finite second moments of $\yvec$ by enforcing $\nupar>2$.
Moreover, when $\nupar$ is large and consequently the likelihood is insensitive to variations in $\nupar$, bounding $\nupar$ from above can alleviate mixing problems.
Consequently, we equip $\nupar$ with a uniform prior $\nupar\sim\Uniform{\nupriorf}{\nupriors}$, where we require $\nupriorf\ge2$.

Finally, in the case of the SV model with leverage, we employ the translated and scaled beta distribution for $\rhopar\in(-1,1)$ as in~\citet{omori2007stochastic}, i.e., $(\rhopar+1)/2\sim\Betadist{\rhopriorf}{\rhopriors}$.
...
The estimation algorithm of SV, SVt, and SVl all resemble the original methodology developed in~\citet{kastner2014ancillarity} for the vanilla SV model.
Namely, to draw from the posterior distribution of $\hvec$ efficiently, the MCMC sampler resorts to approximate mixture representations of Equations~\ref{eq:vanillasv},~\ref{eq:heavysv}, and~\ref{eq:svleverage} similar to the ones in \citet{kim1998stochastic} and \citet{omori2007stochastic}.
...
$\Permmat$ of size $\nfac\times\nfac$, we find another valid decomposition $\Sigmay_t=\Loadings^\prime\Sigmafac^\prime_t(\Loadings^\prime)^\top+\Sigmaidi_t$, where $\Loadings^\prime=\Loadings\Permmat^\top$ and
...
%
The \stochvol{} package provides means for fitting univariate SV, SVt, and SVl models via its sampling routines \fct{svsample}, \fct{svtsample}, and \fct{svlsample}, respectively.
...
+    traceplot(res_svl$beta[, i])
+    densplot(res_svl$beta[, i], show.obs = FALSE)
+  }
\end{Sinput}
\end{Schunk}
...
(rho+1)/2 ~ Beta(a0 = 4, b0 = 4)

Posterior draws of parameters (thinning = 1):
            mean    sd     5%    50%    95% ESS
mu        -2.033 0.410 -2.687 -2.023 -1.384 509
phi        0.976 0.010  0.958  0.977  0.991 207
sigma      0.277 0.043  0.211  0.274  0.350  85
rho       -0.266 0.098 -0.420 -0.271 -0.096 241
exp(mu/2)  0.369 0.076  0.261  0.364  0.501 509
sigma^2    0.079 0.024  0.044  0.075  0.122  85
\end{Soutput}
\end{Schunk}
For brevity, we set \code{showlatent = FALSE} in order not to print all the 1027 latent states.
The output shows the length of the burn-in and the number of draws, the prior specification of the parameters, and a concise summary of the marginal posterior distributions of the parameters $\mupar$, $\phipar$, $\sigmapar$, and $\rhopar$, and additionally of the level of the volatility $\exp(\mupar/2)$ and of $\sigma^2$.
...
The user can influence the shown quantiles by passing a sequence of values between 0 and 1 to \fct{svsample}/\fct{svtsample}/\fct{svlsample} via the argument \code{quantiles}.
...
R> ts.plot(cbind(t(apply(pred_svl$y, 2, quantile, c(0.05, 0.5, 0.95))),
...
Concerning the common model parameters $\mupar$, $\phipar$, and $\sigmapar$, all of \fct{svsample}, \fct{svtsample}, and \fct{svlsample} expect through their input arguments \code{priormu}, \code{priorphi}, and \code{priorsigma} values for $(\mupriorf,\sqrt{\mupriors})$, $(\phipriorf,\phipriors)$, and $\sigmaprior$, respectively.
Furthermore, all sampling functions accept the argument \code{priorbeta} to set the prior of regression coefficients by providing $(\betapriorf,\sqrt{\betapriors})$.
The prior of $\nupar$ can be influenced in \fct{svtsample} by passing $(\nupriorf,\nupriors)$ as the argument \code{priornu}.
Finally, \fct{svlsample} takes the \code{numeric} sequence $(\rhopriorf,\rhopriors)$ through the input argument \code{priorrho}.
...
+    priorsigma = 1, priornu = c(2, 50), priorbeta = c(0, 10000))
R> svlsample(CHF_logret, priormu = c(0, 100), priorphi = c(5, 1.5),
+    priorsigma = 1, priorrho = c(4, 4), priorbeta = c(0, 10000))
\end{Sinput}
\end{Schunk}
...
The default is to draw 10000 elements after a burnin of 1000, which in our experience is enough for most applications.
As for the initial values, we find them to be of minor importance as the samplers tend to mix well enough in real world data sets.
Nevertheless, \code{startpara} and \code{startlatent} provide a way to set them.
...
R> m <- 6
R> n <- 1000
R> y <- 100 * logret(tail(exrates[, seq_len(m)], n + 1), demean = TRUE)
R> plot.ts(y)
\end{Sinput}
\end{Schunk}
%
...
\begin{Sinput}
R> library(factorstochvol)
R> res <- fsvsample(y, factors = 2, draws = 10000, thin = 10, quiet = TRUE)
\end{Sinput}
\end{Schunk}
...
\begin{Schunk}
\begin{Sinput}
R> det_n <- apply(cov_n[,,,1], 3, det)
R> ts.plot(log(det_n))
R> acf(log(det_n), main = "")
\end{Sinput}
\end{Schunk}
...
\begin{Soutput}
     [,1] [,2] [,3] [,4] [,5] [,6]
[1,] 1000 1000  899  744 1000 1000
[2,] 1000 1000  566 1069 1000  817
[3,]  899  566  825  512  335  342
[4,]  744 1069  512  864  613  837
[5,] 1000 1000  335  613 1097 1000
[6,] 1000  817  342  837 1000 1000
\end{Soutput}
\end{Schunk}
...
AUD  1.00 0.61 -0.02  0.03 0.09 0.46
CAD  0.61 1.00  0.08  0.01 0.11 0.51
CHF -0.02 0.08  1.00 -0.03 0.03 0.09
CZK  0.03 0.01 -0.03  1.00 0.00 0.01
DKK  0.09 0.11  0.03  0.00 1.00 0.09
GBP  0.46 0.51  0.09  0.01 0.09 1.00
\end{Soutput}
\begin{Sinput}
...
\begin{Soutput}
     AUD  CAD  CHF  CZK  DKK  GBP
AUD 0.00 0.15 0.18 0.04 0.07 0.16
CAD 0.15 0.00 0.14 0.02 0.07 0.17
CHF 0.18 0.14 0.00 0.04 0.03 0.11
CZK 0.04 0.02 0.04 0.00 0.01 0.02
DKK 0.07 0.07 0.03 0.01 0.00 0.06
...
Two functions called \fct{update\_sv} and \fct{update\_svl} are exported and programmers have the possibility to access the samplers in \stochvol{} directly from \proglang{C++} after linking to the compiled package.
