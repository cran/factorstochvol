...
<<svrunmodel, eval=TRUE, echo=TRUE>>=
library("stochvol")
data("exrates")
...
<<svtrunmodel, echo=TRUE, eval=TRUE, dependson="svrunmodel">>=
...
<<svlrunmodel, echo=2:5, eval=TRUE, dependson="svtrunmodel">>=
set.seed(1)
X <- cbind(constant = 1,
...
<<plotsvlpred, echo=2:3, eval=TRUE, fig.height=3.5, fig.width=10>>=
opar <- par(mgp = c(1.7, 0.5, 0))
obs_CHF <- 100 * logret(exrates$CHF[pred_ind])
ts.plot(cbind(t(apply(pred_svl$y, 2, quantile, c(0.05, 0.5, 0.95))),
